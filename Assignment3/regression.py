...
def fit_regression_model(X, y):
    ...
    for epoch in range(1, num_epochs):
        ...
        if epoch % 100 == 0:
            print(f"Loss {epoch}-", previous_loss)

        if (losses[-2] - losses[-1]) < 0.01:
            print(f"Stopping loop as there is no change in loss at {epoch}")# Change this condition to stop the training when the loss is not changing much.
            break
